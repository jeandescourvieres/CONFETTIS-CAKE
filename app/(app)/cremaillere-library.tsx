...
import { CREMAILLERE_GROUPS, type CremaillereSubGroup } from '../../src/constants/cremaillereMessages';
...
export default function CremaillereLibraryScreen() {
  const C = useColors();
  ...
  const [selectedGroup, setSelectedGroup] = useState<CremaillereSubGroup | null>(null);
  ...
  const totalMessages = CREMAILLERE_GROUPS.reduce((a, g) => a + g.messages.length, 0);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: C.background }]} edges={['top']}>
      <View style={styles.topbar}>
        ...
        <Text style={styles.topbarTitle}>🏠 Crémaillère</Text>
      ...
      {view === 'groups' && (
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={[styles.heroBanner, { backgroundColor: ACCENT_LIGHT, borderWidth: 1.5, borderColor: ACCENT + '40' }]}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.heroTitle, { color: ACCENT }]}>Bienvenue chez toi</Text>
              <Text style={[styles.heroSub, { color: C.onSurfaceVariant }]}>
                Pour {firstName} · {totalMessages} messages · 5 ambiances
              </Text>
            </View>
          </View>
          {CREMAILLERE_GROUPS.filter((g) => g.messages.length > 0).map((group) => (
            <TouchableOpacity
              key={group.id}
          ...
          ListHeaderComponent={
            <View style={[styles.heroBanner, { backgroundColor: ACCENT_LIGHT, borderWidth: 1.5, borderColor: ACCENT + '40' }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.heroSub, { color: C.onSurfaceVariant }]}>
                  {selectedGroup.messages.length} messages · pour {firstName}
                </Text>
              </View>
            </View>
          }
