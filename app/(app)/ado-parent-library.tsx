...
import { ADO_PARENT_GROUPS, type BirthdaySubGroup } from '../../src/constants/birthdayMessages';
...
export default function AdoParentLibraryScreen() {
  const C = useColors();
  const router = useRouter();

  const { contactId, contactName, contactCivilite } = useLocalSearchParams<{
    contactId: string;
    contactName: string;
    contactCivilite?: string;
  }>();

  const firstName = (() => { const p = (contactName ?? '').trim().split(/\s+/); const f = p.filter((w) => !(w === w.toUpperCase() && /[A-Z]/.test(w))); return f.join(' ') || p[0] || contactName || 'Parent'; })();
  const parentRole = contactCivilite === 'M.' ? 'Papa' : contactCivilite === 'Mme' ? 'Maman' : firstName;

  const [view, setView] = useState<ViewMode>('groups');
  const [selectedGroup, setSelectedGroup] = useState<BirthdaySubGroup | null>(null);
  ...
  const fillMessage = (text: string): string =>
    text.replace(/\{prenom\}/gi, parentRole);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: C.background }]} edges={['top']}>
      <View style={styles.topbar}>
        ...
        <Text style={styles.topbarTitle}>🫂 Ado → Parent</Text>
      ...
      {view === 'groups' && (
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={[styles.heroBanner, { backgroundColor: ACCENT_LIGHT, borderWidth: 1.5, borderColor: ACCENT + '40' }]}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.heroTitle, { color: ACCENT }]}>Ado écrit à son parent</Text>
              <Text style={[styles.heroSub, { color: C.onSurfaceVariant }]}>
                Pour {parentRole} · {ADO_PARENT_GROUPS.reduce((a, g) => a + g.messages.length, 0)} messages · 5 ambiances
              </Text>
            </View>
          </View>
          {ADO_PARENT_GROUPS.map((group) => (
            <TouchableOpacity
              key={group.id}
      ...
      {view === 'messages' && selectedGroup && (
        <FlatList
          ...
          ListHeaderComponent={
            <View style={[styles.heroBanner, { backgroundColor: ACCENT_LIGHT, borderWidth: 1.5, borderColor: ACCENT + '40' }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.heroSub, { color: C.onSurfaceVariant }]}>
                  {selectedGroup.messages.length} messages · pour {parentRole}
                </Text>
              </View>
            </View>
          }
          renderItem={({ item }) => {
            ...
            const previewLines = lines.slice(1, 3).join(' ').slice(0, 180);
            return (
