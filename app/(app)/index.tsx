...
function getWeatherTheme(emoji: string): { grad: [string, string]; txt: string; sub: string } {
  if (emoji.includes('☀'))  return { grad: ['#FCD34D', '#F97316'], txt: '#7C2D12', sub: 'rgba(124,45,18,0.7)' };
  if (emoji.includes('🌤'))  return { grad: ['#93C5FD', '#3B82F6'], txt: '#1E3A5F', sub: 'rgba(30,58,95,0.7)' };
  if (emoji.includes('⛅'))  return { grad: ['#BAE6FD', '#7DD3FC'], txt: '#0C4A6E', sub: 'rgba(12,74,110,0.7)' };
  if (emoji.includes('☁'))  return { grad: ['#9CA3AF', '#6B7280'], txt: '#fff',    sub: 'rgba(255,255,255,0.7)' };
  if (emoji.includes('🌦'))  return { grad: ['#60A5FA', '#2563EB'], txt: '#fff',    sub: 'rgba(255,255,255,0.7)' };
  if (emoji.includes('🌧'))  return { grad: ['#3B82F6', '#1E40AF'], txt: '#fff',    sub: 'rgba(255,255,255,0.7)' };
  if (emoji.includes('⛈'))  return { grad: ['#4B5563', '#1F2937'], txt: '#fff',    sub: 'rgba(255,255,255,0.7)' };
  if (emoji.includes('❄') || emoji.includes('🌨')) return { grad: ['#E0F2FE', '#BAE6FD'], txt: '#0C4A6E', sub: 'rgba(12,74,110,0.7)' };
  if (emoji.includes('🌫'))  return { grad: ['#D1D5DB', '#9CA3AF'], txt: '#374151', sub: 'rgba(55,65,81,0.7)' };
  if (emoji.includes('🌙'))  return { grad: ['#1E293B', '#0F172A'], txt: '#fff',    sub: 'rgba(255,255,255,0.7)' };
  return                             { grad: ['#6EE7B7', '#059669'], txt: '#064E3B', sub: 'rgba(6,78,59,0.7)' };
}
...
const INTRO_ACCORDION: { emoji: string; title: string; body: string }[] = [
  ...
  { emoji: '🎨', title: 'Portraits IA pour tes contacts', body: 'Génère un portrait illustré unique pour chacun de tes contacts ! L\'IA s\'inspire de leur âge, leur signe astrologique, leur animal chinois, leur couleur préférée et leur personnalité pour créer une illustration artistique — jamais la même deux fois. Accès depuis la fiche de chaque contact.' },
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {homeMode === 'simple' && (
          <>
            {/* ── Hero ── */}
            <LinearGradient colors={appTheme.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.hero}>
              <ConfettiBar />
              <View style={styles.heroBtnRow}>
                <TouchableOpacity style={styles.cakeBtn} onPress={() => router.push('/welcome' as never)} activeOpacity={0.75}>
                  <Text style={styles.cakeBtnText}>Accueil</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.searchBtn} onPress={() => router.push('/(app)/search' as never)} activeOpacity={0.75}>
                    ...
                    <TouchableOpacity onPress={() => setWeatherOpen(v => !v)} activeOpacity={0.85} style={{ paddingHorizontal: 16, paddingTop: 6, paddingBottom: 10 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                        <View style={{ flex: 1, gap: 4 }}>
                          ...
                          <View style={{ alignSelf: 'flex-start', backgroundColor: 'rgba(0,0,0,0.15)', borderRadius: Radii.full, paddingVertical: 3, paddingHorizontal: 8, marginTop: 2 }}>
                            <Text style={{ fontFamily: 'BeVietnamPro_700Bold', fontSize: Typography.sm, color: '#fff' }}>{comment}</Text>
                          </View>
                        </View>
                  ...
                  <View style={{ backgroundColor: Colors.surface, borderRadius: Radii.lg, padding: 10, gap: 8 }}>
                    ...
                    {zodiacSeasonContacts.length > 0 && (
                      <View style={{ gap: 4 }}>
                        ...
                        {zodiacSeasonContacts.slice(0, 4).map((c) => {
                        ...
                        {zodiacSeasonContacts.length > 4 && (
                          <Text style={{ fontFamily: 'BeVietnamPro_400Regular', fontSize: Typography.xs, color: Colors.onSurfaceVariant }}>
                            et {zodiacSeasonContacts.length - 4} autre{zodiacSeasonContacts.length - 4 > 1 ? 's' : ''}…
                          </Text>
                        )}
                      </View>
                    )}
            ...
            {/* Carte animaux */}
            <View style={{ borderRadius: Radii.xl, backgroundColor: '#064E3B', padding: Spacing[4], gap: 8, marginTop: 12, marginHorizontal: 16, borderWidth: 3, borderColor: '#FDE047' }}>
...
function makeStyles(C: ReturnType<typeof useColors>) {
  ...
  bottomBanner: { marginHorizontal: Spacing[4], marginTop: Spacing[4], backgroundColor: '#5B21B6', borderRadius: Radii.xl, padding: Spacing[4], flexDirection: 'row', alignItems: 'center', gap: 12, borderBottomWidth: 5, borderBottomColor: '#3B0764' },
