...
// ── Dégradés météo ───────────────────────────────────────────────────────────
type WeatherStyle = { gradient: [string, string]; dark: boolean };

function weatherStyle(emoji: string): WeatherStyle {
  if (emoji === '☀️')                   return { gradient: ['#f59e0b', '#fbbf24'], dark: false };
  if (emoji === '🌤️')                   return { gradient: ['#3b82f6', '#fbbf24'], dark: false };
  if (emoji === '☁️')                   return { gradient: ['#6b7280', '#9ca3af'], dark: false };
  if (emoji === '🌫️')                   return { gradient: ['#6b7280', '#9ca3af'], dark: false };
  if (emoji === '🌧️' || emoji === '🌦️') return { gradient: ['#1d4ed8', '#3b82f6'], dark: false };
  if (emoji === '❄️' || emoji === '🌨️') return { gradient: ['#5b8db8', '#93c5fd'], dark: false };
  if (emoji === '⛈️')                   return { gradient: ['#4c1d95', '#7c3aed'], dark: false };
  if (emoji === '🌙')                   return { gradient: ['#1e293b', '#475569'], dark: false };
  return { gradient: ['#3b82f6', '#7c3aed'], dark: false };
}
...
export default function HomeScreen() {
  ...
  const hasCheckedFeaturesIntro = useRef(false);

  // Redirige vers l'intro features si jamais vue
  useFocusEffect(useCallback(() => {
    if (hasCheckedFeaturesIntro.current) return;
    hasCheckedFeaturesIntro.current = true;
    SecureStore.getItemAsync(FEATURES_INTRO_KEY).then((val) => {
      if (!val) router.push('/(app)/features-intro' as never);
    });
  }, [router]));
  ...
  // ── Blocs partagés ──────────────────────────────────────────────────────────
  const imminentCards = imminentEvents.map((event) => {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {/* ── Toggle mode ──────────────────────────────── */}
        <TouchableOpacity style={styles.modeToggle} onPress={toggleHomeMode} activeOpacity={0.75}>
          <Text style={[styles.modeToggleText, { color: homeMode === 'simple' ? Colors.onSurfaceVariant : Colors.primary }]}>
            {homeMode === 'simple' ? '🔓 Passer en mode Initié' : '🎓 Revenir en mode Apprentissage'}
          </Text>
        </TouchableOpacity>

        {/* ════════════════════════════════════════════════
            MODE APPRENTISSAGE — vue épurée
        ════════════════════════════════════════════════ */}
        {homeMode === 'simple' && (
          <>
            {/* CTA principale */}
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Text style={styles.mainCtaText}>✨ Créer un message magique</Text>
            </TouchableOpacity>

            {/* Événements imminents (seulement si il y en a) */}
            {imminentEvents.length > 0 && (
              <View style={styles.imminentSection}>
                <Text style={styles.imminentTitle}>Aujourd'hui et demain :</Text>
                {imminentCards}
              </View>
            )}

            {/* Résumé agenda — 1 ligne */}
            <TouchableOpacity
              style={styles.agendaSummary}
              onPress={() => router.push('/(app)/calendar' as never)}
              activeOpacity={0.75}
            >
              <Text style={styles.agendaSummaryEmoji}>📅</Text>
              <View style={{ flex: 1 }}>
                <Text style={styles.agendaSummaryTitle}>Ton agenda</Text>
                <Text style={styles.agendaSummaryText}>
                  {(birthdayEvents.length + nameDayEvents.length) === 0
                    ? 'Aucun événement dans les 30 prochains jours'
                    : `${birthdayEvents.length + nameDayEvents.length} événement${(birthdayEvents.length + nameDayEvents.length) > 1 ? 's' : ''} à venir`}
                </Text>
              </View>
              <Text style={styles.agendaSummaryChevron}>›</Text>
            </TouchableOpacity>

            {/* Accès rapide — 4 tuiles */}
            <View style={[styles.quickGrid, { marginTop: Spacing[4] }]}>
              <QuickAction emoji="👥" label="Mes contacts"    onPress={() => router.push('/(app)/contacts' as never)} />
              <QuickAction emoji="🎴" label="Cartes animées"  onPress={() => router.push('/(app)/cards/' as never)} />
              <QuickAction emoji="🎁" label="Cagnottes"       onPress={() => router.push('/(app)/pot' as never)} />
              <QuickAction emoji="📊" label="Tableau de bord" onPress={() => router.push('/(app)/dashboard' as never)} />
            </View>

            {/* Bande info — météo + zodiac + aide */}
            <View style={styles.infoStrip}>
              {weather && (
                <View style={styles.infoChip}>
                  <Text style={styles.infoChipEmoji}>{weather.emoji}</Text>
                  <Text style={styles.infoChipText}>{weather.temp}°C</Text>
                </View>
              )}
              <TouchableOpacity
                style={[styles.infoChip, { flex: 1 }]}
                onPress={() => router.push('/(app)/zodiac-season' as never)}
                activeOpacity={0.75}
              >
                <Text style={styles.infoChipEmoji}>{currentZodiacSign.emoji}</Text>
                <Text style={styles.infoChipText} numberOfLines={1}>{currentZodiacSign.name}</Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                ...
                <Text style={styles.infoChipText}>Aide</Text>
              </TouchableOpacity>
            </View>
          </>
        )}
        ...
        {/* ════════════════════════════════════════════════
            MODE INITIÉ — vue complète (home_v1_pleine)
        ════════════════════════════════════════════════ */}
        {homeMode === 'advanced' && (
          <>
            {/* Météo locale */}
            {weather && (() => {
              const ws = weatherStyle(weather.emoji);
              return (
                <LinearGradient colors={ws.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.weatherCard}>
            ...
            <TouchableOpacity style={styles.zodiacSignBlock} onPress={() => router.push('/(app)/zodiac-season' as never)} activeOpacity={0.8}>
              ...
              <Text style={styles.zodiacSignPortrait}>{getZodiacPortrait(currentZodiacSign.name)}</Text>
              <Text style={styles.zodiacSignChevron}>›</Text>
            </TouchableOpacity>

            {/* Carte intro masquable */}
            {introVisible
              ? <IntroCard onHelp={() => router.push('/(app)/help' as never)} onDismiss={handleDismissIntro} />
            ...
            <View style={styles.imminentSection}>
              ...
              {imminentEvents.length > 0 ? imminentCards : (
            ...
            {/* Accès rapide — 9 tuiles */}
            <SectionHeader title={t('home.quickAccess')} />
            <View style={styles.quickGrid}>
              <QuickAction emoji="💬✨" label={t('home.quickNewMessage')} onPress={() => router.push('/(app)/create/' as never)} accent />
              <QuickAction emoji="🎴" label="Je découvre mes cartes animées" onPress={() => router.push('/(app)/cards/' as never)} />
              <QuickAction emoji="👥" label={t('home.quickContacts')} onPress={() => router.push('/(app)/contacts' as never)} />
              <QuickAction emoji="🎁" label={t('home.quickPot')} onPress={() => router.push('/(app)/pot/new' as never)} />
              <QuickAction emoji="📋" label="Mes cagnottes en cours" onPress={() => router.push('/(app)/pot' as never)} />
              <QuickAction emoji="🌟" label={t('home.quickReferral')} onPress={() => router.push('/(app)/referral/' as never)} />
            ...
            {recentSent.length > 0 && (
              <>
                <TouchableOpacity style={styles.collapsibleBtn} onPress={() => setShowSent((v) => !v)} activeOpacity={0.75}>
                  <Text style={styles.collapsibleBtnText}>✉️ Mes derniers messages envoyés</Text>
                  <Text style={styles.collapsibleBtnArrow}>{showSent ? '▾' : '▸'}</Text>
                </TouchableOpacity>
                {showSent && (
                  <View style={styles.recentList}>
                    ...
                    <TouchableOpacity onPress={() => router.push({ pathname: '/(app)/creations', params: { filter: 'sent' } } as never)} style={{ alignSelf: 'center', marginTop: 4 }}>
                      <Text style={styles.collapsibleSeeAll}>{t('common.seeAll')} →</Text>
                    </TouchableOpacity>
                  </View>
                )}
              </>
            ...
            {recentDrafts.length > 0 && (
              <>
                <TouchableOpacity style={styles.collapsibleBtn} onPress={() => setShowDrafts((v) => !v)} activeOpacity={0.75}>
                  <Text style={styles.collapsibleBtnText}>📝 Mes brouillons</Text>
                  <Text style={styles.collapsibleBtnArrow}>{showDrafts ? '▾' : '▸'}</Text>
                </TouchableOpacity>
                {showDrafts && (
                  <View style={styles.recentList}>
                    ...
                    <TouchableOpacity onPress={() => router.push({ pathname: '/(app)/creations', params: { filter: 'draft' } } as never)} style={{ alignSelf: 'center', marginTop: 4 }}>
                      <Text style={styles.collapsibleSeeAll}>{t('common.seeAll')} →</Text>
                    </TouchableOpacity>
                  </View>
                )}
...
function makeStyles(C: ReturnType<typeof useColors>) {
  ...
  quickAction: {
    width: (SCREEN_WIDTH - Spacing[4] * 2 - 20) / 3,
    backgroundColor: Colors.white,
    borderRadius: Radii.xl,
  ...
  weatherInfo:  { gap: 2, marginRight: 4 },
  weatherTemp: {
    fontFamily: 'BeVietnamPro_700Bold',
    fontSize: Typography.md,
    color: '#fff',
  ...
  zodiacSignPortrait: { fontSize: 38, opacity: 0.9 },
  ...
  agendaSummaryEmoji: { fontSize: 24 },
  agendaSummaryTitle: {
    fontFamily: 'BeVietnamPro_700Bold',
    fontSize: Typography.md,
    color: Colors.onSurface,
  ...
  agendaSummaryChevron: { fontSize: 22, color: Colors.outlineVariant },

  // ── Bande info météo + zodiac + aide (mode simple) ───────────────────────
  infoStrip: {
    flexDirection: 'row',
    alignItems: 'center',
    marginHorizontal: Spacing[4],
